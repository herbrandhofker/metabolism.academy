...
class Details extends LitElement {
    ...
    doChapter(chapter, teller) {
        ...
        <div id=${teller+"_"}>
            ...
            ${chapter.video?  this.doButton(teller+"_", chapter.video, chapter.h1) : null}         
    ...
    doOl(ol, teller) {
        if (ol)
            return html`
            <ol>
                ${ol.map((item, index) =>(item.video)? html`<li id = ${teller + "_" + index} >${item.li} ${this.doButton(teller + "_" + index, item.video, item.li)}</li>`: null)}
            </ol>`; 
        return null;
    }

    doButton(parentId, videoData, title) {
        return html`
        <button id=${parentId} @click=${e =>{ this.showVideo(parentId, videoData, title)}}>Details</button>
       `
    }

    showVideo(parentId, videoData, title) {
        const parent = this.shadowRoot.getElementById(parentId);
        if (!parent) { console.error("parent of video is null"); return; }
        this.createVideo(parent, videoData, title);      
    }
